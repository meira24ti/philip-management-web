...
function buildLaporanHTML(judul, tipe, data, dari, sampai) {
  ...
  if (tipe === "penjualan") {
    const rows = (data.transaksi || []).map(t => `
      <tr>
        <td>${escapeHtml(t.nama_jalan)}, ${escapeHtml(t.kota)}</td><td>${escapeHtml(t.kategori || "-")}</td>
        <td>${escapeHtml(t.jenis)}</td><td>${escapeHtml(t.tanggal_transaksi)}</td>
    ...
    const tipeRows = (data.byTipe || []).map(t => `
      <tr><td>${escapeHtml(t.kategori)}</td><td>${t.jumlah}</td>
    ...
    const typeRows = (data.byTipe || []).map(t => `
      <tr><td>${escapeHtml(t.kategori || "-")}</td><td>${t.jumlah || 0}</td>
    ...
    const trenRows = (data.trenBulan || []).map(t => `
      <tr><td>${escapeHtml(t.bulan)}</td><td>${t.total_transaksi || 0}</td><td>${fmt(t.total_komisi)}</td></tr>`).join("");
    body = `
      <div class="kpi-row">
  ...
  <body>
    <div class="header"><h1>Philip Real Estate</h1><p>${escapeHtml(judul)}</p><p>Periode: ${escapeHtml(dari)} s/d ${escapeHtml(sampai)}</p></div>
