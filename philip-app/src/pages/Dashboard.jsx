...
function PropertyCard({ property }) {
  ...
  return (
    <Link to={"/property/" + property.id} className="group block overflow-hidden rounded-2xl bg-white shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="p-3">
        <p className="mb-1 truncate text-[10px] font-bold uppercase tracking-wide text-red-800 sm:text-xs">{property.kategori || "Properti"}</p>
...
export default function Dashboard() {
  ...
  const filtered = properties.filter((property) => {
    ...
    return (!query || searchable.includes(query)) &&
      ...
      (!typeFilter || String(property.kategori || "").toLowerCase() === typeFilter) &&
